...
The browser bridge exposes local-only `/api/health`, `/api/scan`, `/api/improve`, `/api/plan`, `/api/import-batch`, `/api/import-states`, and `/api/import-events` endpoints so cmux or another in-app browser can exercise the same scan, planning, improvement, resumable import, saved cursor, and import activity code paths without Tauri IPC.
