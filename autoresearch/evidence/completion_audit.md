...
- `npm run check`: PASS, 10 quiet UI helper tests passed, Vite production build completed, 36 library tests plus 13 CLI tests passed, and strict clippy passed.
- UI warning notice: PASS, `ScanResult.warnings` renders through the existing notice pattern with a warning variant.
- `cargo check`: PASS.
- `cargo test`: PASS, 36 library tests plus 13 CLI tests passed.
