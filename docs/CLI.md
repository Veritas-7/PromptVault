...
- `work-ai-provider-status` reports OpenAI, GLM, and local `codex exec` work-management provider readiness without exposing secret values. Configured OpenAI/GLM rows list the specific work-management capabilities they can attempt (`work-summary`, `work-log-extraction`, `work-log-normalization`, `session-evidence-proposals`). Codex detection is readiness-only until a source-traced runner is wired behind review gates, so its capabilities remain empty.
