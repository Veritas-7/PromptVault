...
- `work-ai-provider-status` reports OpenAI, GLM, and local `codex exec` work-management provider readiness without exposing secret values. Codex detection is readiness-only until a source-traced runner is wired behind review gates.
