...
- `serve` starts a local browser bridge for cmux/in-app browser QA. It exposes `/api/health`, `/api/scan`, `/api/improve`, `/api/plan`, and `/api/import-batch` on the requested local address.
