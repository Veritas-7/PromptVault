...
async function runBrowserQa() {
  ...
  let workSessionEvidenceProposalsUiWarning = "";
  ...
  let workLogNormalizationProposalsWarning = "";
  ...
  try {
    ...
    await page.waitForFunction((previousCodexProcessedFiles) => {
      ...
      const comparison = document.querySelector('[data-work-session-index-backfill-comparison="true"]')
        ?.textContent ?? "";
      ...
      return meta.includes("until-complete")
        ...
        && comparison.includes("백필 비교")
        && comparison.includes("표준")
        && comparison.includes("긴")
        && comparison.includes("권장 긴 이어 백필")
    ...
    workSessionIndexBackfill = {
      ...
      continued: {
        ...
        comparison: (await page.locator('[data-work-session-index-backfill-comparison="true"]').textContent())
          ?.trim() ?? "",
    ...
    await page.waitForFunction((previousCodexProcessedFiles) => {
      ...
      return meta.includes("until-complete")
        ...
        && planned.includes("긴 이어 백필 예상")
        && comparison.includes("백필 비교")
        && comparison.includes("긴 백필이")
        && comparison.includes("회 절감")
        && guidance.includes("체크포인트 계획")
    ...
    workSessionIndexBackfill.comparisonAfterBatch500 =
      (await page.locator('[data-work-session-index-backfill-comparison="true"]').textContent())?.trim() ?? "";
    ...
    if (workSessionEvidenceProposalsUiMeta.includes("경고")) {
      await page.waitForFunction(() => {
        const text = document.querySelector('[data-work-session-evidence-proposals-warning="true"]')
          ?.textContent ?? "";
        return text.includes("경고") && (text.includes("fallback") || text.includes("provider"));
      }, undefined, { timeout: 30000 });
      workSessionEvidenceProposalsUiWarning =
        (await page.locator('[data-work-session-evidence-proposals-warning="true"]').textContent())?.trim() ?? "";
    }
    ...
    if (workLogNormalizationProposalsMeta.includes("경고")) {
      await page.waitForFunction(() => {
        const text = document.querySelector('[data-work-log-normalization-proposals-warning="true"]')
          ?.textContent ?? "";
        return text.includes("경고") && (text.includes("fallback") || text.includes("provider"));
      }, undefined, { timeout: 30000 });
      workLogNormalizationProposalsWarning =
        (await page.locator('[data-work-log-normalization-proposals-warning="true"]').textContent())?.trim() ?? "";
    }
