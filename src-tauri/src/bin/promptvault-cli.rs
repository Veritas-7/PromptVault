    ...
    match command.as_str() {
        ...
        "work-session-evidence-proposals" => {
            ...
            while let Some(arg) = iter.next() {
                match arg.as_str() {
                    ...
                    other => {
                        return Err(format!(
                            "unknown work-session-evidence-proposals argument: {other}"
                        )
                        .into())
                    }
                }
            ...
            let result = run_project_work_session_evidence_proposals(
            ...
            println!("PromptVault session evidence proposals");
            println!("database: {}", result.database_path);
            println!("provider: {}", result.provider);
            println!("provider_runtime: {}", result.provider_runtime);
            println!("used_ai: {}", result.used_ai);
            println!("candidates: {}", result.total_candidate_count);
        ...
        "work-session-evidence-review-queue" => {
            ...
            if let Some(review_state_filter) = &review_state_filter {
                println!("review_state: {review_state_filter}");
            }
            ...
            println!("deferred: {}", result.deferred_count);
        ...
        "work-session-evidence-review-apply" => {
            ...
            while let Some(arg) = iter.next() {
                match arg.as_str() {
                    ...
                    other => {
                        return Err(format!(
                            "unknown work-session-evidence-review-apply argument: {other}"
                        )
                        .into())
                    }
                }
            }
            let result = run_project_work_session_evidence_review_apply(
            ...
            println!("PromptVault session evidence review apply");
            println!("database: {}", result.database_path);
            ...
            println!("total_reviewed_items: {}", result.total_reviewed_item_count);
            println!("returned_items: {}", result.returned_item_count);
            for item in &result.items {
                println!(
                    "\n{} · {} · {} · {}",
        ...
        "work-session-evidence-source-search" => {
            ...
            while let Some(arg) = iter.next() {
                match arg.as_str() {
                    ...
                    other => {
                        return Err(format!(
                            "unknown work-session-evidence-source-search argument: {other}"
                        )
                        .into())
                    }
                }
            ...
            let result = run_project_work_session_evidence_source_search(
                ProjectWorkSessionEvidenceSourceSearchOptions {
                    source_path: source_path
                        .ok_or("work-session-evidence-source-search requires --source-path")?,
                    query: query.ok_or("work-session-evidence-source-search requires --query")?,
                    limit,
                    max_lines,
                },
            )?;
            ...
            println!("PromptVault source session evidence search");
            println!("source: {}", result.source_path);
            println!(
                "query_terms: {} · {}",
                result.query_term_count, result.query
            );
            println!("scanned_lines: {}", result.scanned_line_count);
            println!("matches: {}", result.matched_line_count);
            println!("returned: {}", result.returned_item_count);
            for item in &result.items {
                println!(
                    "\nline {} · score {} · {}",
        ...
        "work-session-index" => {
            ...
            println!("until_complete: {}", result.until_complete);
        ...
        "work-log-candidates" => {
            ...
            println!(
                ...
                result.risk_blocked_candidate_count
            );
        ...
        "work-log-review-queue" => {
            ...
            while let Some(arg) = iter.next() {
                match arg.as_str() {
                    ...
                    other => {
                        return Err(
                            format!("unknown work-log-review-queue argument: {other}").into()
                        )
                    }
                }
            }
            ...
            println!("PromptVault project work log review queue");
            println!("database: {}", result.database_path);
            println!("total_items: {}", result.total_items);
            println!("returned_items: {}", result.returned_item_count);
            println!("synced_candidates: {}", result.synced_candidate_count);
            println!("stale_candidates: {}", result.stale_candidate_count);
            println!("pending_ai_review: {}", result.pending_ai_review_count);
            println!("risk_blocked: {}", result.risk_blocked_count);
            println!("stale: {}", result.stale_count);
            println!("approved: {}", result.approved_count);
            println!("rejected: {}", result.rejected_count);
            if let Some(review_state_filter) = &review_state_filter {
            ...
            for item in &result.items {
                println!(
                    "\n{} · {} · {} · {}",
                    item.candidate_id, item.project, item.review_state, item.provider_route
                );
                println!("- reason: {}", item.review_reason);
                println!("- source: {} · {}", item.source_file, item.source_path);
                println!("- excerpt: {}", item.excerpt);
            }
        }
        "work-log-review-queue-update" => {
            let json = take_flag(&mut args, "--json");
            let mut limit = None;
            let mut database_path = None;
            let mut candidate_id = None;
            ...
            while let Some(arg) = iter.next() {
                match arg.as_str() {
                    ...
                    other => {
                        return Err(format!(
                            "unknown work-log-review-queue-update argument: {other}"
                        )
                        .into())
                    }
                }
            ...
            let result =
                run_project_work_log_review_queue_update(ProjectWorkLogReviewQueueUpdateOptions {
                    database_path,
                    limit,
                    candidate_id: candidate_id
                        .ok_or("work-log-review-queue-update requires --candidate-id")?,
                    review_state: review_state
                        .ok_or("work-log-review-queue-update requires --state")?,
                    review_reason,
                })?;
            if json {
                println!("{}", serde_json::to_string_pretty(&result)?);
            ...
            println!("PromptVault project work log review queue update");
            println!("database: {}", result.database_path);
            println!("total_items: {}", result.total_items);
            println!("pending_ai_review: {}", result.pending_ai_review_count);
            println!("risk_blocked: {}", result.risk_blocked_count);
        ...
        "work-log-extract" => {
            ...
            let approved_review_queue_only = take_flag(&mut args, "--approved-review-queue");
            let mut limit = None;
            let mut database_path = None;
            let mut iter = args.into_iter();
            while let Some(arg) = iter.next() {
            ...
            let result = run_project_work_log_extraction_proposals(
                ProjectWorkLogExtractionProposalsOptions {
                    ...
                    approved_review_queue_only: Some(approved_review_queue_only),
                },
            )
            .await?;
            if json {
        ...
        "work-log-freeze" => {
            ...
            while let Some(arg) = iter.next() {
                match arg.as_str() {
                    ...
                    other => {
                        return Err(format!("unknown work-log-freeze argument: {other}").into())
                    }
                }
            }
            let result = run_project_work_log_freeze(ProjectWorkLogFreezeOptions {
                database_path,
                limit,
            })?;
            if json {
            ...
            println!("PromptVault frozen live project/date work log rows");
            println!("root: {}", result.root_path);
            println!("provider: {}", result.provider);
            println!("candidates: {}", result.candidate_count);
            println!("saved: {}", result.accepted_count);
            if let Some(persistence) = &result.persistence {
                println!(
                    "persisted: {} rows (total {}) to {}",
                    persistence.saved_item_count,
                    persistence.total_saved_item_count,
                    persistence.database_path
                );
        ...
        "work-log-runs" => {
            ...
            while let Some(arg) = iter.next() {
                match arg.as_str() {
                    ...
                    other => return Err(format!("unknown work-log-runs argument: {other}").into()),
                }
            }
            let result =
            ...
            println!("PromptVault work log extraction run history");
            println!("database: {}", result.database_path);
            println!("runs: {}", result.total_runs);
            println!("returned: {}", result.returned_run_count);
            for run in &result.runs {
                ...
                println!("- provider_runtime: {}", run.provider_runtime);
                println!("- used_ai: {}", run.used_ai);
                println!(
                    "- candidates: {} · accepted: {} · rejected: {}",
                    run.candidate_count, run.accepted_count, run.rejected_count
                );
                println!(
                    "- saved: {} · total_saved: {}",
                    run.saved_item_count, run.total_saved_item_count
                );
                if !run.candidate_ids.is_empty() {
                    println!("- candidate_ids: {}", run.candidate_ids.join(", "));
                }
                if !run.warnings.is_empty() {
                    println!("- warnings: {}", run.warnings.join("; "));
                }
        ...
        "work-log-normalization-candidates" => {
            ...
            while let Some(arg) = iter.next() {
                match arg.as_str() {
                    ...
                    other => {
                        return Err(format!(
                            "unknown work-log-normalization-candidates argument: {other}"
                        )
                        .into())
                    }
                }
            }
            let result = run_project_work_log_normalization_candidates(
            ...
            println!("PromptVault work log normalization candidates");
            println!("database: {}", result.database_path);
            println!("report_items: {}", result.report_total_items);
            println!("candidates: {}", result.total_candidate_count);
            println!("returned: {}", result.returned_candidate_count);
            for candidate in &result.candidates {
                println!(
                    "\n{} · {} · {}",
                    candidate.candidate_id, candidate.date, candidate.project
                );
                println!("- reason: {}", candidate.reason);
                ...
                println!(
                    ...
                    candidate
                        .best_ai_confidence
                        .map(|confidence| format!(" · best AI confidence {:.2}", confidence))
                        .unwrap_or_default()
                );
                if !candidate.risk_flags.is_empty() {
                    println!("- risks: {}", candidate.risk_flags.join(", "));
                }
                println!("  {}", candidate.source_path);
            }
        }
        "work-log-normalization-proposals" => {
            ...
            while let Some(arg) = iter.next() {
                match arg.as_str() {
                    ...
                    other => {
                        return Err(format!(
                            "unknown work-log-normalization-proposals argument: {other}"
                        )
                        .into())
                    }
                }
            }
            let result = run_project_work_log_normalization_proposals(
            ...
            println!("PromptVault work log normalization proposals");
            println!("database: {}", result.database_path);
            println!(
                "provider: {} · runtime: {}{} · used_ai: {}",
                result.provider,
                result.provider_runtime,
                result
                    .provider_model
            ...
            for proposal in &result.proposals {
                println!(
                    "\n{} · {} · {}",
                    proposal.candidate_id, proposal.date, proposal.project
                );
                ...
                println!("- normalized_title: {}", proposal.normalized_title);
                println!("- normalized_status: {}", proposal.normalized_status);
                println!("- normalized_evidence: {}", proposal.normalized_evidence);
                println!("- confidence: {:.2}", proposal.confidence);
                println!("- original_title: {}", proposal.original_title);
                println!(
                    "- items: {} · sessions: {} · saved: {} · ai saved: {}{}",
                    proposal.work_item_count,
                    proposal.session_evidence_count,
                    proposal.saved_extraction_count,
                    proposal.ai_saved_extraction_count,
                    proposal
                        .best_ai_confidence
                        .map(|confidence| format!(" · best AI confidence {:.2}", confidence))
                        .unwrap_or_default()
                ...
                if !proposal.risk_flags.is_empty() {
                    println!("- risks: {}", proposal.risk_flags.join(", "));
                }
                println!("  {}", proposal.source_path);
            }
            if !result.warnings.is_empty() {
        ...
        "work-log-normalized-items" => {
            ...
            while let Some(arg) = iter.next() {
                match arg.as_str() {
                    ...
                    other => {
                        return Err(
                            format!("unknown work-log-normalized-items argument: {other}").into(),
                        )
                    }
                }
            }
            ...
            println!("PromptVault work log normalized items");
            println!("database: {}", result.database_path);
            println!("total_items: {}", result.total_items);
            println!("returned_items: {}", result.returned_item_count);
            for item in &result.items {
        ...
        "work-summary" => {
            ...
            let result = run_project_work_summary(ProjectWorkSummaryOptions {
                ...
                include_saved_extractions: Some(include_saved_extractions),
                extraction_limit,
...
fn work_session_evidence_source_audit_help_text() -> String {
    [
        ...
        "",
        "Usage:",
        "  promptvault-cli work-session-evidence-source-audit [--limit N>0] [--row-filter FILTER] [--review-state pending_review|stale|deferred|approved|rejected|all] [--nearby-limit N>0] [--source-limit N>0] [--max-lines N>0] [--session-limit N>0] [--database PATH] [--sync-candidates] [--refresh-session-index] [--json]",
        "",
        "Purpose:",
...
fn work_session_evidence_review_queue_help_text() -> String {
    [
        ...
        "",
        "Usage:",
        "  promptvault-cli work-session-evidence-review-queue [--sync-candidates] [--refresh-session-index] [--limit N>0] [--row-filter FILTER] [--review-state pending_review|stale|deferred|approved|rejected|all] [--session-limit N>0] [--database PATH] [--json]",
        "",
        "Purpose:",
        ...
        "  Approve, defer, or reject one row with work-session-evidence-review-queue-update.",
        "",
        "Examples:",
...
fn work_session_evidence_review_queue_update_help_text() -> String {
    [
        ...
        "",
        "Usage:",
        "  promptvault-cli work-session-evidence-review-queue-update --candidate-id ID --state approved|deferred|rejected [--reason TEXT] [--source-review-json JSON|--source-review-file PATH] [--limit N>0] [--database PATH] [--json]",
        "",
        "Purpose:",
        "  Marks one persisted session-evidence review queue row approved, deferred, or rejected with an audit reason.",
...
fn work_session_evidence_review_apply_help_text() -> String {
    [
        ...
        "  Run work-session-evidence-review-queue first to inspect pending, stale, deferred, approved, and rejected rows.",
...
fn help_text() -> String {
    concat!(
        ...
        "  work-session-evidence-review-queue [--limit N>0] [--row-filter FILTER] [--review-state pending_review|stale|deferred|approved|rejected|all] [--session-limit N>0] [--database PATH] [--sync-candidates] [--refresh-session-index] [--json]\n",
        "  work-session-evidence-review-queue-update --candidate-id ID --state approved|deferred|rejected [--reason TEXT] [--source-review-json JSON|--source-review-file PATH] [--limit N>0] [--database PATH] [--json]\n",
        ...
        "  work-session-evidence-source-audit [--limit N>0] [--row-filter FILTER] [--review-state pending_review|stale|deferred|approved|rejected|all] [--nearby-limit N>0] [--source-limit N>0] [--max-lines N>0] [--session-limit N>0] [--database PATH] [--sync-candidates] [--refresh-session-index] [--json]\n",
        ...
        "  work-session-evidence-review-queue persists unresolved session-evidence candidates into an operator review queue and marks disappeared candidates stale when a full candidate sync is available; --row-filter narrows read-only queue views by project/day evidence shape and --review-state narrows by operator decision state without changing sync behavior.\n",
        "  work-session-evidence-review-queue-update marks one persisted session-evidence candidate approved, deferred, or rejected with an audit reason; source-proposal approvals can pass copied trace metadata with --source-review-json or --source-review-file.\n",
...
struct ProjectWorkSummaryBridgeOptions {
    ...
    include_saved_extractions: Option<bool>,
...
#[derive(serde::Deserialize)]
struct ProjectWorkSessionEvidenceProposalsBridgePayload {
...
#[derive(serde::Deserialize)]
struct ProjectWorkSessionEvidenceReviewApplyBridgePayload {
...
#[derive(serde::Deserialize)]
struct ProjectWorkLogReviewQueueBridgePayload {
    options: Option<ProjectWorkLogReviewQueueOptions>,
}

#[derive(serde::Deserialize)]
struct ProjectWorkLogReviewQueueUpdateBridgePayload {
...
#[derive(serde::Deserialize)]
struct ProjectWorkLogFreezeBridgePayload {
...
#[derive(serde::Deserialize)]
struct ProjectWorkLogRunsBridgePayload {
    options: Option<ProjectWorkLogExtractionRunsOptions>,
}

#[derive(serde::Deserialize)]
struct ProjectWorkLogNormalizationCandidatesBridgePayload {
...
impl ProjectWorkSummaryBridgeOptions {
    fn into_project_work_summary_options(self, database_path: &Path) -> ProjectWorkSummaryOptions {
        ProjectWorkSummaryOptions {
            ...
            include_saved_extractions: self.include_saved_extractions,
...
fn handle_bridge_route(
    ...
    match (request.method.as_str(), path) {
        ...
        ("POST", "/api/work-session-evidence-proposals") => {
            let payload = serde_json::from_str::<ProjectWorkSessionEvidenceProposalsBridgePayload>(
                &request.body,
            )?;
            let mut options = payload.options.unwrap_or_default();
            options
            ...
            let runtime = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()?;
            let result = runtime.block_on(run_project_work_session_evidence_proposals(options))?;
            write_json_response(stream, 200, &result)
        }
        ...
        ("POST", "/api/work-session-evidence-review-apply") => {
            let payload = serde_json::from_str::<ProjectWorkSessionEvidenceReviewApplyBridgePayload>(
                &request.body,
            )?;
            let mut options = payload.options.unwrap_or_default();
            options
        ...
        ("POST", "/api/work-session-evidence-source-search") => {
            let payload = serde_json::from_str::<
                ProjectWorkSessionEvidenceSourceSearchBridgePayload,
            >(&request.body)?;
        ...
        ("POST", "/api/work-log-review-queue") => {
            ...
            options
                .database_path
                .get_or_insert_with(|| bridge_database_path(database_path));
            let result = run_project_work_log_review_queue(options)?;
            write_json_response(stream, 200, &result)
        }
        ("POST", "/api/work-log-review-queue/update") => {
            let payload = serde_json::from_str::<ProjectWorkLogReviewQueueUpdateBridgePayload>(
                &request.body,
            )?;
            let mut options = payload.options;
            options
                .database_path
                .get_or_insert_with(|| bridge_database_path(database_path));
            let result = run_project_work_log_review_queue_update(options)?;
            write_json_response(stream, 200, &result)
        }
        ...
        ("POST", "/api/work-log-freeze") => {
            let payload = serde_json::from_str::<ProjectWorkLogFreezeBridgePayload>(&request.body)?;
            let mut options = payload.options.unwrap_or_default();
            options
                .database_path
                .get_or_insert_with(|| bridge_database_path(database_path));
            let result = run_project_work_log_freeze(options)?;
            write_json_response(stream, 200, &result)
        }
        ...
        ("POST", "/api/work-log-runs") => {
            let payload = serde_json::from_str::<ProjectWorkLogRunsBridgePayload>(&request.body)?;
            let mut options = payload.options.unwrap_or_default();
            options
                .database_path
                .get_or_insert_with(|| bridge_database_path(database_path));
            let result = run_list_project_work_log_extraction_runs(options)?;
            write_json_response(stream, 200, &result)
        }
        ("POST", "/api/work-log-normalization-candidates") => {
            let payload = serde_json::from_str::<ProjectWorkLogNormalizationCandidatesBridgePayload>(
                &request.body,
            )?;
            let mut options = payload.options.unwrap_or_default();
            options
                .database_path
                .get_or_insert_with(|| bridge_database_path(database_path));
            let result = run_project_work_log_normalization_candidates(options)?;
            write_json_response(stream, 200, &result)
        }
        ("POST", "/api/work-log-normalization-proposals") => {
            let payload = serde_json::from_str::<ProjectWorkLogNormalizationProposalsBridgePayload>(
                &request.body,
            )?;
            let mut options = payload.options.unwrap_or_default();
            options
        ...
        ("POST", "/api/work-log-normalized-items") => {
            let payload =
                serde_json::from_str::<ProjectWorkLogNormalizedItemsBridgePayload>(&request.body)?;
            let mut options = payload.options.unwrap_or_default();
            options
                .database_path
                .get_or_insert_with(|| bridge_database_path(database_path));
            let result = run_list_project_work_log_normalized_items(options)?;
            write_json_response(stream, 200, &result)
        }
...
mod tests {
    ...
    #[test]
    fn bridge_routes_work_session_evidence_proposals_validation_errors() {
        let response = bridge_response_for(
    ...
    fn bridge_routes_work_session_evidence_review_queue_validation_errors() {
        ...
        let source_search_response = bridge_response_for(
            "/api/work-session-evidence-source-search",
            r#"{"options":{"source_path":"/tmp/session.jsonl","query":"PromptVault","limit":0}}"#,
        );

        assert!(source_search_response.starts_with("HTTP/1.1 400 Bad Request"));
    ...
    fn bridge_routes_work_session_index_validation_errors() {
        ...
        let until_complete_response = bridge_response_for(
            "/api/work-session-index",
            r#"{"options":{"until_complete":true}}"#,
        );

        assert!(until_complete_response.starts_with("HTTP/1.1 400 Bad Request"));
    ...
    fn bridge_routes_work_log_item_validation_errors() {
        ...
        assert!(
            project_response.contains("work-log item project filter requires a non-empty value")
        );
    }

    #[test]
    fn bridge_routes_work_log_run_validation_errors() {
        let response = bridge_response_for("/api/work-log-runs", r#"{"options":{"limit":0}}"#);

        assert!(response.starts_with("HTTP/1.1 400 Bad Request"));
        assert!(response.contains("work-log run limit requires a positive integer"));
        assert!(response.contains("Access-Control-Allow-Origin: *"));
    }

    #[test]
    fn bridge_routes_work_log_normalization_candidate_validation_errors() {
        let response = bridge_response_for(
        ...
        assert!(response.starts_with("HTTP/1.1 400 Bad Request"));
        assert!(
            response.contains("work-log normalization candidate limit requires a positive integer")
        );
        assert!(response.contains("Access-Control-Allow-Origin: *"));
        ...
        assert!(session_response.contains(
            "work-log normalization candidate session_limit requires a positive integer"
        ));
    }

    #[test]
    fn bridge_routes_work_log_normalization_proposal_validation_errors() {
        let response = bridge_response_for(
        ...
        assert!(response.starts_with("HTTP/1.1 400 Bad Request"));
        assert!(
            response.contains("work-log normalization proposal limit requires a positive integer")
        );
        assert!(response.contains("Access-Control-Allow-Origin: *"));
        ...
        assert!(session_response
            .contains("work-log normalization proposal session_limit requires a positive integer"));
    }
    ...
    fn bridge_routes_work_log_normalization_review_queue_validation_errors() {
        ...
        let normalized_items_response = bridge_response_for(
            "/api/work-log-normalized-items",
            r#"{"options":{"limit":0}}"#,
        );

        assert!(normalized_items_response.starts_with("HTTP/1.1 400 Bad Request"));
        assert!(normalized_items_response
            .contains("work-log normalized item limit requires a positive integer"));
        assert!(normalized_items_response.contains("Access-Control-Allow-Origin: *"));
    }
    ...
    fn bridge_routes_work_log_freeze_validation_errors() {
        ...
        assert!(response.starts_with("HTTP/1.1 400 Bad Request"));
        assert!(response.contains("work-log freeze limit requires a positive integer"));
        assert!(response.contains("Access-Control-Allow-Origin: *"));
    }

    #[test]
    fn bridge_routes_work_log_review_queue_validation_errors() {
        let response =
            bridge_response_for("/api/work-log-review-queue", r#"{"options":{"limit":0}}"#);

        assert!(response.starts_with("HTTP/1.1 400 Bad Request"));
        assert!(response.contains("work-log review queue limit requires a positive integer"));
        assert!(response.contains("Access-Control-Allow-Origin: *"));

        let update_response = bridge_response_for(
            "/api/work-log-review-queue/update",
            r#"{"options":{"candidate_id":"work-log-Test-a1","review_state":"pending_ai_review"}}"#,
        );

        assert!(update_response.starts_with("HTTP/1.1 400 Bad Request"));
        assert!(
            update_response.contains("work-log review queue state must be approved or rejected")
        );
        assert!(update_response.contains("Access-Control-Allow-Origin: *"));
    }
    ...
    fn help_text_documents_cli_validation_rules() {
        ...
        assert!(help.contains(
            "work-session-evidence-review-queue-update --candidate-id ID --state approved|deferred|rejected"
        ));
        ...
        assert!(help.contains(
            "work-session-evidence-source-audit [--limit N>0] [--row-filter FILTER] [--review-state pending_review|stale|deferred|approved|rejected|all]"
        ));
        assert!(
            help.contains(
                "work-session-index [--limit N>0] [--batch-files 1..500] [--max-batches N>0] [--until-complete] [--confirm-long-run] [--database PATH] [--reset] [--json]"
        ...
        assert!(help.contains("work-log-freeze [--limit N>0] [--database PATH] [--json]"));
        assert!(help.contains(
            "work-log-items [--limit N>0] [--database PATH] [--date YYYY-MM-DD] [--project NAME] [--json]"
        ));
        ...
        assert!(help.contains("--approved-review-queue reads only operator-approved"));
        ...
        assert!(
            help.contains("--include-saved-extractions merges stored accepted AI extraction rows")
        );
    ...
    fn work_session_evidence_review_queue_help_documents_sync_review_safety() {
        ...
        assert!(help.contains("pending, stale, deferred, approved, rejected"));
    ...
    fn work_session_evidence_review_queue_update_help_documents_source_review_safety() {
        ...
        assert!(help.contains("--candidate-id ID --state approved|deferred|rejected"));
        assert!(help.contains("--source-review-json JSON|--source-review-file PATH"));
        assert!(help.contains("approved, deferred, or rejected with an audit reason"));
    ...
    fn parse_source_review_arg_accepts_json_or_file_once() {
        let payload = serde_json::json!({
            ...
            "project": "PromptVault",
            "date": "2026-06-10",
            "source_path": "/Users/example/.codex/sessions/2026/06/10/rollout-promptvault.jsonl",
