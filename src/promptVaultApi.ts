...
export async function loadProjectWorkStatusExport(
  options: ProjectWorkStatusExportOptions = {},
): Promise<ProjectWorkStatusExportResult> {
  if (hasTauriInvoke()) {
    return invoke<ProjectWorkStatusExportResult>("project_work_status_export", { options });
  }
  return postBridge<ProjectWorkStatusExportResult>(
    "/api/work-status-export",
    { options },
    parseProjectWorkStatusExportResult,
  );
}

export async function loadProjectWorkSessionEvidenceCandidates(
  options: ProjectWorkSessionEvidenceCandidatesOptions = {},
): Promise<ProjectWorkSessionEvidenceCandidatesResult> {
  if (hasTauriInvoke()) {
    return invoke<ProjectWorkSessionEvidenceCandidatesResult>(
      "project_work_session_evidence_candidates",
      { options },
    );
  }
  return postBridge<ProjectWorkSessionEvidenceCandidatesResult>(
...
export async function loadProjectWorkSessionEvidenceProposals(
  options: ProjectWorkSessionEvidenceProposalsOptions = {},
): Promise<ProjectWorkSessionEvidenceProposalsResult> {
  if (hasTauriInvoke()) {
    return invoke<ProjectWorkSessionEvidenceProposalsResult>(
      "project_work_session_evidence_proposals",
      { options },
    );
  }
  return postBridge<ProjectWorkSessionEvidenceProposalsResult>(
    "/api/work-session-evidence-proposals",
    { options },
    parseProjectWorkSessionEvidenceProposalsResult,
  );
}
...
function projectWorkStatusExportRowsWithinResult(value: unknown): boolean {
  ...
  for (const row of value.rows) {
    if (!isRecord(row)
      || !isPositiveSafeInteger(row.work_item_count)
      || !isNonNegativeSafeInteger(row.session_evidence_count)) {
      return false;
    }
    workItemCount += row.work_item_count;
    sessionEvidenceCount += row.session_evidence_count;
    if (!Number.isSafeInteger(workItemCount)
      || !Number.isSafeInteger(sessionEvidenceCount)
      || workItemCount > value.report_total_items
      || sessionEvidenceCount > value.report_session_evidence_count) {
      return false;
    }
  }
  return value.rows.length === value.returned_row_count;
}
...
function parseProjectWorkStatusExportResult(value: unknown): ProjectWorkStatusExportResult {
  if (!isRecord(value)
    ...
    || !isNonNegativeSafeIntegerAtMost(value.report_project_count, value.report_total_items)
    || !isNonNegativeSafeIntegerAtMost(value.report_date_count, value.report_total_items)
    || !isNonNegativeSafeInteger(value.report_files_seen)
    || !isNonNegativeSafeInteger(value.report_session_scan_prompt_count)
    || !isNonNegativeSafeInteger(value.report_session_evidence_count)
    || !isNonNegativeSafeIntegerAtMost(
      value.report_unique_session_evidence_count,
      value.report_session_evidence_count,
    )
    ...
    || !projectWorkStatusExportRowsWithinResult(value)
    || !isNonBlankStringArray(value.warnings)) {
    throw new Error(MALFORMED_BRIDGE_RESPONSE_MESSAGE);
  }
  return value as unknown as ProjectWorkStatusExportResult;
}

function isProjectWorkSessionEvidenceCandidate(value: unknown): boolean {
  return isRecord(value)
    && isNonBlankString(value.candidate_id)
    && isNonBlankString(value.date)
    && isNonBlankString(value.project)
...
function parseProjectWorkSessionEvidenceCandidatesResult(
  value: unknown,
): ProjectWorkSessionEvidenceCandidatesResult {
  if (!isRecord(value)
    || !isTimestampString(value.generated_at)
    ...
    || !isPositiveSafeInteger(value.session_limit_used)
    || !isNonNegativeSafeInteger(value.total_candidate_count)
    || !isNonNegativeSafeIntegerAtMost(value.returned_candidate_count, value.total_candidate_count)
    || !isNonNegativeSafeInteger(value.report_total_rows)
    || !isNonNegativeSafeInteger(value.report_total_items)
    || !isNonNegativeSafeIntegerAtMost(value.report_project_count, value.report_total_items)
    || !isNonNegativeSafeIntegerAtMost(value.report_date_count, value.report_total_items)
    || !isNonNegativeSafeInteger(value.report_files_seen)
    ...
    || value.report_session_evidence_index_count > value.report_session_evidence_index_total_count
    || !isNonBlankString(value.report_session_evidence_mode)
    || !isNonNegativeSafeIntegerAtMost(value.bounded_session_limit_count, value.report_total_rows)
    || !isNonNegativeSafeIntegerAtMost(
      value.unresolved_after_full_index_count,
      value.report_total_rows,
    )
    || value.total_candidate_count !== value.unresolved_after_full_index_count
    || !isNonNegativeSafeIntegerAtMost(value.needs_title_normalization_count, value.report_total_rows)
    || !Array.isArray(value.candidates)
    || value.candidates.length !== value.returned_candidate_count
    || !value.candidates.every(isProjectWorkSessionEvidenceCandidate)
    || !recordStringFieldValuesAreUnique(value.candidates, "candidate_id")
    || !isNonBlankStringArray(value.warnings)) {
    throw new Error(MALFORMED_BRIDGE_RESPONSE_MESSAGE);
  }
...
function isProjectWorkSessionEvidenceProposalKind(value: unknown): boolean {
  return [
    "source_log_trace",
...
function isProjectWorkSessionEvidenceProposal(value: unknown): boolean {
  return isRecord(value)
    && isNonBlankString(value.candidate_id)
    && isNonBlankString(value.project)
    && isNonBlankString(value.date)
    && isNonBlankString(value.source_path)
    && isNonBlankString(value.source_file)
    && isProjectWorkSourceFileRole(value.source_role)
    && isNonBlankString(value.candidate_reason)
    ...
    && value.confidence <= 1
    && typeof value.accepted === "boolean"
    && (value.rejection_reason === null || isNonBlankString(value.rejection_reason))
    && isPositiveSafeInteger(value.work_item_count)
    && value.session_evidence_audit === "unresolved-after-full-index"
    && typeof value.needs_title_normalization === "boolean"
    && Array.isArray(value.top_titles)
    ...
    && isNonBlankStringArray(value.risk_flags)
    && (!value.accepted || value.rejection_reason === null)
    && (value.accepted || value.rejection_reason !== null);
}

function parseProjectWorkSessionEvidenceProposalsResult(
  value: unknown,
): ProjectWorkSessionEvidenceProposalsResult {
  if (!isRecord(value)
    || !isTimestampString(value.generated_at)
    || !isNonBlankString(value.database_path)
    || !isNonBlankString(value.provider)
    || !(value.provider_model === null || isNonBlankString(value.provider_model))
    || !isNonBlankString(value.provider_runtime)
    || typeof value.used_ai !== "boolean"
    || !isNonNegativeSafeInteger(value.total_candidate_count)
    || !isNonNegativeSafeIntegerAtMost(value.returned_proposal_count, value.total_candidate_count)
    || !isNonNegativeSafeIntegerAtMost(value.accepted_count, value.returned_proposal_count)
    || !isNonNegativeSafeIntegerAtMost(value.rejected_count, value.returned_proposal_count)
    || Number(value.accepted_count) + Number(value.rejected_count) !== value.returned_proposal_count
    || !isNonNegativeSafeInteger(value.report_total_rows)
    || !isNonNegativeSafeInteger(value.report_total_items)
    || !isNonNegativeSafeIntegerAtMost(value.report_project_count, value.report_total_items)
    || !isNonNegativeSafeIntegerAtMost(value.report_date_count, value.report_total_items)
    ...
    || !Array.isArray(value.proposals)
    || value.proposals.length !== value.returned_proposal_count
    || !value.proposals.every(isProjectWorkSessionEvidenceProposal)
    || !recordStringFieldValuesAreUnique(value.proposals, "candidate_id")
    || !isNonBlankStringArray(value.warnings)) {
    throw new Error(MALFORMED_BRIDGE_RESPONSE_MESSAGE);
  }
  return value as unknown as ProjectWorkSessionEvidenceProposalsResult;
}
...
function isProjectWorkSessionEvidenceReviewQueueItem(value: unknown): boolean {
  return isRecord(value)
    ...
    && isTimestampString(value.first_seen_at)
    && isTimestampString(value.last_seen_at)
    && ["pending_review", "stale", "deferred", "approved", "rejected"].includes(String(value.review_state))
    && isNonBlankString(value.review_reason)
    && isNonBlankString(value.date)
    && isNonBlankString(value.project)
    && ["active", "session-supported", "progress-log-only"].includes(String(value.operational_status))
...
function parseProjectWorkSessionEvidenceReviewQueueResult(
  ...
  if (!isRecord(value)
    ...
    || !isNonNegativeSafeIntegerAtMost(value.deferred_count, value.total_items)
