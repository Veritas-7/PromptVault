  ...
  return {
    ...
    summaries: [{
      ...
      citations: [{
        ...
        status: "done",
        source_path: "/Users/example/Ai/System/10_Projects/01_apps-products/PromptVault/working.md",
...
function projectWorkStatusExportPayload(overrides = {}) {
  return {
    ...
    rows: [{
      ...
      latest_source_path: "/Users/example/Ai/System/10_Projects/01_apps-products/PromptVault/working.md",
...
function projectWorkSessionEvidenceSourceReviewPayload(overrides = {}) {
  return {
    ...
    source_cwd: "/Users/example/Ai/System/10_Projects/01_apps-products/PromptVault",
...
function projectWorkSessionEvidenceReviewApplyPayload(overrides = {}) {
  const approvedItem = {
...
function projectWorkSessionEvidenceNearbyPayload(overrides = {}) {
  return {
    ...
    items: [{
      ...
      cwd: "/Users/example/Ai/System/10_Projects/01_apps-products/PromptVault",
...
function projectWorkLogCoveragePayload(overrides = {}) {
  return {
    ...
    files: [{
      ...
      project: "CareVault",
      source_path: "/Users/example/Ai/System/10_Projects/01_apps-products/CareVault/docs/progress.md",
...
function projectWorkLogCandidatesPayload(overrides = {}) {
  return {
    ...
    candidates: [{
      ...
      project: "CareVault",
      source_path: "/Users/example/Ai/System/10_Projects/01_apps-products/CareVault/workingd.md",
...
function projectWorkLogNormalizationProposalsPayload(overrides = {}) {
  return {
    generated_at: "2026-06-09T00:00:00Z",
    database_path: "/tmp/promptvault.sqlite",
    ...
    proposals: [{
      ...
      original_evidence: "2026-06-09: Backfilled workingd notes",
      normalized_title: "Backfilled workingd notes",
      normalized_status: "current",
      normalized_evidence: "2026-06-09: Backfilled workingd notes",
      confidence: 0.5,
      accepted: false,
...
function projectWorkLogNormalizationApplyPayload(overrides = {}) {
  return {
    generated_at: "2026-06-09T00:00:00Z",
    database_path: "/tmp/promptvault.sqlite",
    approved_queue_count: 1,
...
test("browser bridge work session evidence review apply posts options and validates rows", async (t) => {
  ...
  globalThis.fetch = async (input, init) => {
    requestPath = String(input);
    requestBody = String(init?.body ?? "");
    return new Response(JSON.stringify(projectWorkSessionEvidenceReviewApplyPayload()), { status: 200 });
  };
  t.after(() => {
    globalThis.fetch = originalFetch;
  });

  const result = await applyProjectWorkSessionEvidenceReviewRows({ limit: 5 });

  assert.match(requestPath, /\/api\/work-session-evidence-review-apply$/);
  assert.deepEqual(JSON.parse(requestBody), { options: { limit: 5 } });
  assert.equal(result.approved_queue_count, 1);
  assert.equal(result.processed_queue_count, 1);
...
test("browser bridge work session evidence review apply rejects impossible counters", async (t) => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async () => new Response(JSON.stringify(projectWorkSessionEvidenceReviewApplyPayload({
...
test("browser bridge work log extraction posts save option and validates persistence", async (t) => {
  ...
  const result = await loadProjectWorkLogExtractionProposals({ limit: 5, ai: true, save: true });
...
test("browser bridge work log extraction posts approved candidate ids for reviewed saves", async (t) => {
  const originalFetch = globalThis.fetch;
  let requestPath = "";
  let requestBody = "";
  globalThis.fetch = async (input, init) => {
  ...
  const result = await loadProjectWorkLogExtractionProposals({
    ...
    ai: true,
    save: true,
    approved_candidate_ids: ["work-log-CareVault-a1b2c3d4e5"],
  });
...
test("browser bridge work log normalization proposals posts options and validates rows", async (t) => {
  const originalFetch = globalThis.fetch;
  let requestPath = "";
  let requestBody = "";
  globalThis.fetch = async (input, init) => {
    requestPath = String(input);
    requestBody = String(init?.body ?? "");
    return new Response(JSON.stringify(projectWorkLogNormalizationProposalsPayload()), { status: 200 });
  };
  t.after(() => {
    globalThis.fetch = originalFetch;
  });
  ...
  assert.equal(result.proposals[0].project, "CareVault");
  assert.equal(result.proposals[0].source_file, "workingd.md");
  assert.equal(result.proposals[0].accepted, false);
  assert.equal(result.proposals[0].rejection_reason, "local_fallback_requires_ai_review");
  assert.deepEqual(result.proposals[0].risk_flags, ["long_base64_like_token"]);
});

test("browser bridge work log normalization proposals reject impossible counters", async (t) => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async () => new Response(JSON.stringify(projectWorkLogNormalizationProposalsPayload({
    accepted_count: 1,
    rejected_count: 1,
  })), { status: 200 });
  t.after(() => {
...
test("browser bridge work log normalization apply posts options and validates rows", async (t) => {
  const originalFetch = globalThis.fetch;
  let requestPath = "";
  let requestBody = "";
  globalThis.fetch = async (input, init) => {
    requestPath = String(input);
    requestBody = String(init?.body ?? "");
    return new Response(JSON.stringify(projectWorkLogNormalizationApplyPayload()), { status: 200 });
  };
  t.after(() => {
    globalThis.fetch = originalFetch;
  });

  const result = await applyProjectWorkLogNormalizationQueue({ limit: 5 });

  assert.match(requestPath, /\/api\/work-log-normalization-apply$/);
  assert.deepEqual(JSON.parse(requestBody), { options: { limit: 5 } });
  assert.equal(result.approved_queue_count, 1);
  assert.equal(result.processed_queue_count, 1);
  assert.equal(result.applied_item_count, 1);
  assert.equal(result.skipped_existing_count, 0);
  assert.equal(result.total_applied_item_count, 1);
  assert.equal(result.items[0].candidate_id, "work-normalize-CareVault-a1b2c3d4e5f6");
  assert.equal(result.items[0].review_reason, "operator_approved_normalization");
});

test("browser bridge work log normalization apply rejects impossible counters", async (t) => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async () => new Response(JSON.stringify(projectWorkLogNormalizationApplyPayload({
    processed_queue_count: 2,
    applied_item_count: 1,
    skipped_existing_count: 0,
  })), { status: 200 });
