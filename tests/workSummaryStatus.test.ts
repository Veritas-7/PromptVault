...
function statusExportResult(overrides: Partial<ProjectWorkStatusExportResult> = {}): ProjectWorkStatusExportResult {
  return {
    ...
    rows: [{
      ...
      latest_source_path: "/Users/example/Ai/System/10_Projects/01_apps-products/CareVault/workingd.md",
...
function extractionCandidate(
  ...
  return {
    ...
    source_path: "/Users/example/Ai/System/10_Projects/01_apps-products/RepoTutorStudio/working.md",
...
test("work status export text exposes project day evidence coverage", () => {
  ...
  assert.equal(
    workStatusExportRowNextActionText(result.rows[0]),
    "다음 조치 · 제목 정규화 큐 검토 후 세션 백필 후 재검증 · 근거 limit 영향",
  );
  assert.equal(
    workStatusExportRowSessionSourcesText(result.rows[1]),
    "세션 소스 · Codex local sessions 9건 · 고유 3건",
  );
  assert.equal(workStatusExportRowNextActionText(result.rows[1]), "다음 조치 · 세션 근거 확인됨");
  assert.equal(
    workStatusExportRowSessionSourcesText({
      ...result.rows[0],
      session_evidence_audit: "unresolved-after-full-index",
  ...
  assert.equal(
    ...
    "다음 조치 · 제목 정규화 큐 검토 후 인접 날짜 세션 후보 검토 · 2026-06-09 · 1일 차이",
  );
  assert.equal(
    workStatusExportRowSessionSourcesText({
      ...result.rows[0],
  ...
  assert.equal(
    ...
    "다음 조치 · 제목 정규화 큐 검토 후 같은 날짜 세션 후보 3건 수동 연결 검토",
  );
  assert.equal(
    workStatusExportRowSessionSourcesText({
      ...result.rows[0],
  ...
  assert.equal(workStatusExportRowFilterLabel("same-date-session-hint"), "같은 날짜 세션 후보");
  assert.equal(workStatusExportRowFilterLabel("near-session-date-hint"), "인접 날짜 세션 후보");
  ...
  assert.deepEqual(
    filterWorkStatusExportRows(rowsWithSessionDateHints, "same-date-session-hint").map((row) => row.project),
    ["SameDateProject"],
  );
  assert.deepEqual(
  ...
  assert.equal(
    ...
    "필터 세션 근거 필요 · 결과 1 / 2행 · 세션근거 필요 1행 · 근거limit 1행 · 전체미해결 0행 · 같은날후보 0행 · 인접후보 0행 · 먼후보 0행 · 제목정규화 필요 1행",
  );
  assert.equal(
    workStatusExportFilterMetaText(
    ...
    "필터 전체 인덱스 미해결 · 결과 1 / 2행 · 세션근거 필요 1행 · 근거limit 0행 · 전체미해결 1행 · 같은날후보 0행 · 인접후보 0행 · 먼후보 0행 · 제목정규화 필요 1행",
  );
  assert.equal(
    workStatusExportFilterMetaText(
      "same-date-session-hint",
      rowsWithSessionDateHints,
      filterWorkStatusExportRows(rowsWithSessionDateHints, "same-date-session-hint"),
    ),
    "필터 같은 날짜 세션 후보 · 결과 1 / 4행 · 세션근거 필요 3행 · 근거limit 0행 · 전체미해결 3행 · 같은날후보 1행 · 인접후보 1행 · 먼후보 1행 · 제목정규화 필요 3행",
  );
  assert.equal(
    workStatusExportFilterMetaText(
    ...
    "필터 인접 날짜 세션 후보 · 결과 1 / 4행 · 세션근거 필요 3행 · 근거limit 0행 · 전체미해결 3행 · 같은날후보 1행 · 인접후보 1행 · 먼후보 1행 · 제목정규화 필요 3행",
  );
...
test("work session evidence source proposal labels explain blocker states", () => {
  ...
  assert.equal(
    workSessionEvidenceSourceProposalBlockerText("source_hit_matches_only_project_or_generic_terms"),
    ...
    recommendedWorkSessionEvidenceSourceSearchSession(sessionEvidenceNearbyResult({
      items: [
        sessionEvidenceNearbyItem({
          ...
          matched_terms: ["promptvault"],
          excerpt:
            "Codex session metadata indexed session project targets: /Users/example/Ai/System/10_Projects/01_apps-products/PromptVault",
        }),
      ],
    })),
    null,
